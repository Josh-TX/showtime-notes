...
export function inverseExpInterp(a: number, b: number, n: number): number {
    return Math.log(n / a) / Math.log(b / a);
}

type NumberArray = Float32Array | Uint8Array | number[] 

export function stepwiseInterp(arr: NumberArray, index: number){
    const lowerInd = index - 0.5;
    const upperInd = index + 0.5;
    if (Math.floor(lowerInd) != Math.floor(index)){
        //n was less than .5
        if (Math.floor(lowerInd) < 0){
            return arr[Math.floor(index)]
        }
        var upperFactor = upperInd - Math.floor(index); 
        var lowerFactor = Math.floor(index) - lowerInd;
        return arr[Math.floor(index)] * upperFactor 
            + arr[Math.floor(lowerInd)] * lowerFactor;
    } else {
        //n was .5 or more
        if (Math.floor(upperInd) > arr.length - 1){
            return arr[Math.floor(index)]
        }
        var upperFactor = upperInd - Math.ceil(index); 
        var lowerFactor = Math.ceil(index) - lowerInd;
        return arr[Math.floor(upperInd)] * upperFactor 
            + arr[Math.floor(index)] * lowerFactor;
    }
}
